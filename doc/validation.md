...
- The repository-standard command still tried to link the `NotEnoughResinsUITests`
  runner in the default DerivedData location and failed with `ld: can't write
  output file`, so this run did not provide reliable signal for the cookie-field
  follow-up itself.

### 2026-03-19 - Task 11 single-line cookie field isolated build verification

- Command:
  `xcodebuild test -scheme NotEnoughResins -destination 'platform=macOS' -skip-testing:NotEnoughResinsUITests -derivedDataPath /tmp/NotEnoughResins-task11-single-line CODE_SIGNING_ALLOWED=NO CODE_SIGN_IDENTITY=-`
- Result: passed

Executed coverage:

- Confirmed the task11 follow-up still passes the unit-test suite after
  replacing the custom AppKit-backed cookie editor with a standard single-line
  field in Preferences.
- Confirmed the scheme can build the updated `NotEnoughResinsUITests` target
  and its `preferences.cookieEditor` lookup against an isolated DerivedData
  path, so the new field type does not break the UI-test target at compile or
  link time.

### 2026-03-19 - Task 11 single-line cookie field UI verification attempt

- Command:
  `xcodebuild test -scheme NotEnoughResins -destination 'platform=macOS' -derivedDataPath /tmp/NotEnoughResins-task11-ui CODE_SIGNING_ALLOWED=NO CODE_SIGN_IDENTITY=- -only-testing:NotEnoughResinsUITests/NotEnoughResinsUITests/testPreferencesSavePersistsAcrossRelaunch -only-testing:NotEnoughResinsUITests/NotEnoughResinsUITests/testPreferencesCanBeReopenedAfterSavingFromMenuBarPanel`
- Result: failed in environment

Observed outcome:

- The targeted UI-test run built successfully with the updated `textFields`
  query, but `NotEnoughResinsUITests-Runner` exited before establishing a test
  connection, so the field-type follow-up still relies on manual confirmation
  plus the passing isolated unit-suite run in this environment.

### 2026-03-19 - Task 11 single-line cookie field documentation verification
...
- Confirmed the task11 follow-up artifact updates for the standard single-line
  cookie field remain clean under the repository Markdown lint configuration.
